...
func main() {
	...
	hotspotNodes := hotspotsNode.SelectNodesDirect("*", "Hotspot")

	// This defines our boundary polygon
	coordX := make([]float64, len(hotspotNodes))
	coordY := make([]float64, len(hotspotNodes))
	coordZ := make([]float64, len(hotspotNodes))

	var minX, minY, maxX, maxY, minZ, maxZ float64
	for i, _ := range hotspotNodes {
		coordX[i] = hotspotNodes[i].Af64("*", "X")
		coordY[i] = hotspotNodes[i].Af64("*", "Y")
		coordZ[i] = hotspotNodes[i].Af64("*", "Z")
...
func pointInPolygon(x, y float64, polyX, polyY []float64) bool {
	j := len(polyX) - 1
	oddNodes := false
...
func visualize(minX, maxX, minY, maxY float64, coordX, coordY, coordZ []float64, plotX, plotY []float64) {
	...
	// for i := 0; i+1 < len(coordX); i += 2 {
	// 	canvas.Line(int(coordX[i])+400, int(coordY[i]), int(coordX[i+1])+400, int(coordY[i+1]), "stroke:blue;stroke-width:5;")
	// }
